...
char *tok_buf = NULL;	/* a pointer to the buffer n which we store the current token */
int tok_bufsize = 0;	/* number of bytes we allocate to the buffer */
int tok_bufindex = -1;  /* the current buffer index */
 ...
 *	if buffer is full, the function extends it
 *	@c: char
 */
 ...
 *
 * Details - takes care of allocating memory for the token struct and text
 *			 also fills the struct member fields
 */
...
/**
 * free_token - frees the memory used by a token struct, and token text
 * @tok: token to free
 */
